...
from pointmass.gym_pointmass.envs import PointMassEnv, PointMassEnvRewardType
...
def experiment(variant):
    base_expl_env = PointMassEnv(n=1, reward_type=variant["reward_type"])
    expl_env = FlatGoalEnv(
        base_expl_env,
        append_goal_to_obs=True
    )
    base_eval_env = PointMassEnv(n=1, reward_type=variant["reward_type"])
...
if __name__ == "__main__":
    ...
    variant = dict(
        reward_type=PointMassEnvRewardType.SPARSE,
    ...
    setup_logger("td3-pointmass-singletask", variant=variant)
    experiment(variant)